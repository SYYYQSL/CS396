...
let animations = [
  ...
  {
    title: "Looping Family",
    description: "Lovely Family",
    isActive: true,
    ...
    draw(p, t) {
      ...
      p.text("👶", x, 180);
      p.textSize(50);
      p.text("👶", x + p.width, 180);
      p.textSize(50);
      p.text("👶", x - p.width, 180);
      p.textSize(50);
      p.text("👩", x, 280);
      p.textSize(50);
      p.text("👩", x + p.width, 280);
      p.textSize(50);
      p.text("👩", x - p.width, 280);
  ...
  {
    title: "Run",
    description: "Regular and fast movement",
    isActive: true,
    ...
    draw(p, t) {
      ...
      let x = radius * Math.tan(this.theta) + centerX;
      let y = radius * Math.cos(this.theta) + centerY;
      let r = 10;

      p.noStroke();
      p.fill(hue, 100, 50);
      p.circle(x, y, r);
      p.circle(x + 10, y + 10, r + 10);
      p.circle(x + 20, y + 20, r + 20);
      p.circle(x + 30, y + 30, r + 30);
    },
  },
  ...
  // Going *around* the screen
  {
    isActive: true,
    title: "Police",
    description: "Police are chasing thieves",
    ...
    draw: function (p, t) {
      ...
      for (var i = 0; i < 40; i++) {
        ...
        let x = (t * 170 + i * 20);
        let y = p.sin(t) * 170 + i * 5 + 100 * p.noise(t + i * 0.1);
        // Loop around!
        x %= p.width;
        y %= p.height;
...

  {
  ...
  
   {
    title: "Spacecraft",
    description:
      "The spaceship wants to go to the sun",
    isActive: true,
    ...
    draw(p, t) {
      ...
      let r = 100;
      let pct = (t % this.loopTime) / this.loopTime;
      let a = p.width * p.noise(t)-100;
      let b = p.height * p.noise(t + 100)-100;
