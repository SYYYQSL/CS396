...
let brushes = [
  ...
  //======================================================
    ...
    mouseDragged() {
      ...
      let x = p.mouseX;
      let y = p.mouseY;
      let pt = [x*p.noise(Math.sin(x)), y*p.noise(Math.cos(y))];
    ...
    draw() {
      ...
      this.points_1.forEach((pt, index) => {
        //
...
//   },
  //======================================================
  {
    ...
    mouseDragged() {
      ...
      p.noFill();
      p.stroke(color0[0]*Math.random(), color0[1]*Math.random(), color0[2] + 50 * Math.random(), 0.8);
      p.beginShape();
      ...
      this.points
        ...
        .forEach(([x, y]) => {
          let dx = 0;
          let dy = 0;
          let da = Math.random()*30
          let db = Math.random()*10
          let dc = Math.random()*50
          let dd = Math.random()*80
          let de = Math.random()*20
          let df = Math.random()*100
          //         What happens if we offset the x and y we are drawing?
          // dx = Math.random()*100
          // dy = Math.random()*10

          p.curveVertex(x + dx, y + dy);
          p.curveVertex(a + da, b + db);
          p.curveVertex(c + dc, d + dd);
          p.curveVertex(e + de, f + df);
        });
